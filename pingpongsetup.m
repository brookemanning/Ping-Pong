...
%mode selector starts HERE ------------------
    ...
    %set callback function for replay
    %create options: easy, medium, hard
    %replay = text(ax,0.5,0.58,'easy','Color','white','FontSize',30,'HorizontalAlignment','center','VerticalAlignment','middle');
    %set(replay, 'ButtonDownFcn', @replay)
    %function replay(~,~)
     %   close(board);
    %end
   ...
   % game parameters: 
    ...
    if score_playerB == winning_score || score_playerA == winning_score
        ...
        if score_playerA == winning_score
            ...
            replay = uicontrol('Parent', board, 'Style', 'pushbutton', 'String', 'Replay', 'Position',[500,300,200,50], 'FontSize',20, 'Callback',@restart);
...
%MEDIUM mode ping pong starts HERE ------------------
   ...
   % game parameters: 
    ...
    if score_playerB == winning_score || score_playerA == winning_score
        ...
        if score_playerA == winning_score
            game_over_message = 'Game is over! Player A wins'; 
            replay = uicontrol('Parent', board, 'Style', 'pushbutton', 'String', 'Replay', 'Position',[500,300,200,50], 'FontSize',20, 'Callback',@restart_medium);
        else 
            game_over_message = 'Game is over! Player B wins';
            replay = uicontrol('Parent', board, 'Style', 'pushbutton', 'String', 'Replay', 'Position',[500,300,200,50], 'FontSize',20, 'Callback',@restart_medium);
        end 
...
%callback function to start a new game of medium ping pong
 function restart_medium(~, ~)
    close(board);
    mediummode_pingpong();
end
...
%HARD mode ping pong starts HERE ------------------
   ...
   % game parameters: 
    ...
    if score_playerB == winning_score || score_playerA == winning_score
        ...
        if score_playerA == winning_score
            game_over_message = 'Game is over! Player A wins'; 
            replay = uicontrol('Parent', board, 'Style', 'pushbutton', 'String', 'Replay', 'Position',[500,300,200,50], 'FontSize',20, 'Callback',@restart_hard);
        else 
            game_over_message = 'Game is over! Player B wins';
            replay = uicontrol('Parent', board, 'Style', 'pushbutton', 'String', 'Replay', 'Position',[500,300,200,50], 'FontSize',20, 'Callback',@restart_hard);
        end 
...
%callback function to start a new game of hard ping pong
