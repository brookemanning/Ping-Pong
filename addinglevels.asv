...
function visual_ball_movement_v4()
...
% WHILE LOOP FOR KEY PRESS/PADDLE MOVEMENT, BALL MOVEMENT, AND SCORING
    ...
    while ~game_over
        ...
        if ballposition(1) - ballradius <= 0|| ballposition(1) + ballradius >= boardwidth %left/right
            % added 3/18 to make ball return to center when it hits left
...
if ballposition(1) <= ballradius*2 && ballvelocity(1) < 0 % if the ball has crossed the left side of the board and is still moving left
    % if the y coordinate of the ball is less than the y coordinate of the
    % paddle 
    % or the y coordinate of the ball is greater than the bottom edge of
    % the paddle 
